...
Phases 1–2 were drafted by both partners in parallel on separate branches to
compare approaches, then merged into a single version:

1. **Stack primitives** — skusakab & jperez-u
2. **Parser** — skusakab & jperez-u
3. **Coordinate compression** — skusakab & jperez-u
4. **Operations** — skusakab & jperez-u

Once both drafts were ready, we compared them, decided together what to keep,
and skusakab integrated the result. The remaining work was divided as follows:

1. **Simple strategy** — jperez-u
2. **Medium strategy** — jperez-u
3. **Complex strategy** — skusakab
4. **Adaptive dispatcher** — skusakab
5. **Benchmark mode** — skusakab
6. **Final integration** — skusakab
7. **Testing** — both
...
Phase 1–2 は、アプローチを比較するため両者が別ブランチで並行ドラフトし、その後 1 つに
統合しました：

1. **スタックのプリミティブ** — skusakab & jperez-u
2. **パーサー** — skusakab & jperez-u
3. **座標圧縮** — skusakab & jperez-u
4. **各種オペレーション** — skusakab & jperez-u

両者のドラフトが揃った後、比較して採用方針を一緒に決め、skusakab が統合しました。残りの
作業は次のように分担しました：

1. **Simple 戦略** — jperez-u
2. **Medium 戦略** — jperez-u
3. **Complex 戦略** — skusakab
4. **Adaptive ディスパッチャ** — skusakab
5. **ベンチマークモード** — skusakab
6. **最終統合** — skusakab
7. **テスト** — 両者
